...
function _transpileWriter(definition, path = '', empty = false, offset_static = 0, offset_dynamic = false) {
    ...
    const serializers = {
        ...
        skillid32: (varName) => varName ? `switch (typeof ${varName}) { case 'object': { buffer.setUint32(${offset(4)}, ((${varName} instanceof this.SkillID) ? ${varName} : (new this.SkillID(${varName}))).toUint32(), true); break; } case 'number': { buffer.setUint32(${offset(4, -4)}, ${varName}, true); break; } default: { buffer.setUint32(${offset(4, -4)}, 0, true); break; } }` : `buffer.setUint32(${offset(4, -4)}, 0, true)`,
        skillid: (varName) => varName ? `switch (typeof ${varName}) { case 'object': { buffer.setBigUint64(${offset(8)}, ((${varName} instanceof this.SkillID) ? ${varName} : (new this.SkillID(${varName}))).toUint64(), true); break; } case 'bigint': { buffer.setBigUint64(${offset(8, -8)}, ${varName}, true); break; } default: { buffer.setBigUint64(${offset(8, -8)}, 0n, true); break; } }` : `buffer.setBigUint64(${offset(8, -8)}, 0n, true)`,
