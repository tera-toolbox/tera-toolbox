...
function GetString(str, tokens) {
    switch (str) {
        // connectionManager
        case 'connectionmanager/error-ETIMEDOUT-1': return `[连接错误]: 无法连接到服务器${tokens.address}:${tokens.port} (连接超时)! 通常导致这个问题的原因:`;
        case 'connectionmanager/error-ETIMEDOUT-2': return '[连接错误]: 网络连接不稳定，或者IP被地区封锁.';
        case 'connectionmanager/error-ETIMEDOUT-3': return '[连接错误]: 游戏服务器正在维护.';
        case 'connectionmanager/error-ECONNABORTED-ECONNRESET-EPIPE-1': return `[连接错误]: ${tokens.code}-与服务器突然断开连接. 通常导致这个问题的原因:`;
        case 'connectionmanager/error-ECONNABORTED-ECONNRESET-EPIPE-2': return '[连接错误]: 由于网络不稳定导致的网络连接中断';
        case 'connectionmanager/error-ECONNABORTED-ECONNRESET-EPIPE-3': return '[连接错误]: 由于使用了作弊/修改/损坏的插件脚本，或修改过的模型文件，导致被服务器踢出.';
        case 'connectionmanager/connected': return `[连接错误] 关联 ${tokens.remote} 至${tokens.remoteAddress}:${tokens.remotePort}`;
        case 'connectionmanager/disconnected': return `[连接错误] ${tokens.remote} 失去连接`;
        ...
        case 'proxy/client-interface-connected': return `[tera工具箱] 客户端 ${tokens.justStarted ? 'connected' : 'reconnected'} (${tokens.region} ,版本号: ${tokens.majorPatchVersion}.${tokens.minorPatchVersion})`;
        case 'proxy/client-interface-disconnected': return `[tera工具箱] 客户端断开连接.`;
        case 'proxy/redirecting-server': return `[tera工具箱] ${tokens.name} (${tokens.region.toUpperCase()}-${tokens.serverId}) 从 ${tokens.listen_ip}:${tokens.listen_port} 定向到 ${tokens.ip}:${tokens.port}`;
        case 'proxy/warning-unmapped-protocol-1': return `[tera工具箱] 警告: 未映射的协议版本 ${tokens.protocolVersion} (${tokens.region.toUpperCase()} 版本号:${tokens.majorPatchVersion}.${tokens.minorPatchVersion}).`;
        ...
        case 'proxy/protocol-loaded': return `[tera工具箱] 已加载协议: ${tokens.protocolVersion} (${tokens.region.toUpperCase()} ,版本号: ${tokens.majorPatchVersion}.${tokens.minorPatchVersion}).`;
        ...
        case 'gui/main/static/tabs/credits/content': return '感谢以下开发人员: <br />SaltyMonkey: 程序安装包封装<br />Mathicha &amp; Pentagon: GUI核心部分编写<br />Foglio: Logo设计<br />Meishu: 原始tera核心便携<br />所有为tera工具箱提供翻译的翻译者<br />所有插件的开发者'; 
        ...
        case 'tera-client-interface/modulemanager/load-module-info-error': return `[插件错误]: 无法获取插件: "${tokens.name}"的信息! 完整的错误日志如下:`;
        case 'tera-client-interface/modulemanager/duplicate-mod-error': return `[插件错误]: 检测到插件: "${tokens.name}"存在重复插件!`;
        case 'tera-client-interface/modulemanager/missing-mod-dependency-error': return `[插件错误]: 插件: ${tokens.name} 需要安装: "${tokens.dependency}" , 但是后者没有安装!`;
        case 'tera-client-interface/modulemanager/mod-conflict-error': return `[插件错误]: 插件: ${tokens.name} 无法在安装: "${tokens.conflict}"的情况下加载!`;
        case 'tera-client-interface/modulemanager/cannot-load-mod-not-installed': return `[插件错误]: 尝试加载未安装的插件: ${tokens.name}`;
        case 'tera-client-interface/modulemanager/cannot-unload-mod-not-installed': return `[插件错误]: 尝试卸载未安装的插件: ${tokens.name}`;
        case 'tera-client-interface/modulemanager/cannot-unload-mod-not-loaded': return `[插件错误]: 尝试卸载未加载的插件: ${tokens.name}`;
        ...
        case 'tera-client-interface/modulemanager/mod-load-error-1': return `[插件错误]: 插件: ${tokens.name} 无法被加载!`;
        case 'tera-client-interface/modulemanager/mod-load-error-2': return `[插件错误]: 请联系该插件的作者: ${tokens.supportUrl}`;
        case 'tera-client-interface/modulemanager/mod-unload-error-1': return `[插件错误]: 插件: ${tokens.name} 无法被卸载!`;
        case 'tera-client-interface/modulemanager/mod-unload-error-2': return `[插件错误]: 请联系该插件的作者: ${tokens.supportUrl}`;
        ...
        case 'tera-network-proxy/connection/dispatch/module/settings-load-error-invalid-format-1': return `[插件错误]: "${tokens.name}的配置文件无效"!`;
        ...
        case 'tera-network-proxy/connection/dispatch/modulemanager/load-module-info-error': return `[插件错误]: 无法获取插件: "${tokens.name}"的信息! 完整的错误日志如下:`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/duplicate-mod-error': return `[插件错误]: 检测到插件: "${tokens.name}"存在重复插件!`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/missing-mod-dependency-error': return `[插件错误]: 插件: ${tokens.name} 需要安装: "${tokens.dependency}" , 但是后者没有安装!`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/mod-conflict-error': return `[插件错误]: 插件: ${tokens.name} 无法在安装: "${tokens.conflict}"的情况下加载!`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/cannot-load-mod-not-installed': return `[插件错误]: 尝试加载未安装的插件: ${tokens.name}`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/cannot-unload-mod-not-installed': return `[插件错误]: 尝试卸载未加载的插件: ${tokens.name}`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/cannot-unload-mod-not-loaded': return `[插件错误]: 尝试卸载未加载的插件: ${tokens.name}`;
        ...
        case 'tera-network-proxy/connection/dispatch/modulemanager/mod-load-error-1': return `[插件错误]: 插件: ${tokens.name} 无法被加载!`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/mod-load-error-2': return `[插件错误]: 请联系该插件的作者: ${tokens.supportUrl}`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/mod-unload-error-1': return `[插件错误]: 插件: ${tokens.name} 无法被卸载!`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/mod-unload-error-2': return `[插件错误]: 请联系该插件的作者: ${tokens.supportUrl}`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/unsupported-def-error-1': return `[插件错误]: 插件: "${tokens.name}" 使用了如下过老/不支持的 数据包:`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/unsupported-def-error-2': return `[插件错误]: ${tokens.name}.${tokens.version}`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/unsupported-def-error-3': return `[插件错误]: 请联系插件作者: ${tokens.supportUrl}`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/cannot-reload-mod-not-installed': return `[插件错误]: 尝试重新加载未安装的插件: ${tokens.name}`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/cannot-reload-mod-not-supported': return `[插件错误]: 尝试重新加载不支持热加载的插件: ${tokens.name}`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/cannot-reload-mod-not-loaded': return `[插件错误]: 尝试重新加载未加载的插件: ${tokens.name}`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/cannot-reload-mod-unload-failed': return `[插件错误]: 插件: ${tokens.name} 重新加载失败.`;
        case 'tera-network-proxy/connection/dispatch/modulemanager/cannot-reload-mod-load-failed': return `[插件错误]: 插件: ${tokens.name} 重新加载失败.`;
