...
function GetString(str, tokens) {
    switch (str) {
        // connection-manager
        case 'connection-manager/error-ETIMEDOUT-1': return `[Verbindung] FEHLER: Verbindung zum Spiele-Server unter ${tokens.address}:${tokens.port} (Time-out) nicht möglich! Häufige Gründe hierfür sind:`;
        case 'connection-manager/error-ETIMEDOUT-2': return '[Verbindung] - Eine instabile Internetverbindung oder eine Regions-IP Sperre';
        case 'connection-manager/error-ETIMEDOUT-3': return '[Verbindung] - Wartung des Spiele-Servers';
        case 'connection-manager/error-ECONNABORTED-ECONNRESET-EPIPE-1': return `[Verbindung] FEHLER: ${tokens.code} - Die Verbindung zum Spiele-Server wurde unerwartet geschlossen. Häufige Gründe hierfür sind:`;
        ...
        case 'loader-gui/mod-installed': return `[Toolbox] Installieren von "${tokens.name}"`;
        case 'loader-gui/mod-uninstalled': return `[Toolbox] Deinstallieren von "${tokens.name}"`;
        ...
        case 'proxy/client-interface-connected': return `[Toolbox] Client wurde ${tokens.justStarted ? 'verbunden' : 'wieder verbunden'} (${tokens.publisher.toUpperCase()} v${tokens.majorPatchVersion}.${tokens.minorPatchVersion})`;
        ...
        case 'proxy/error-cannot-load-protocol': return `FEHLER: Protokoll-Version ${tokens.protocolVersion} (${tokens.publisher.toUpperCase()} v${tokens.majorPatchVersion}.${tokens.minorPatchVersion}) konnte nicht initialisiert werden!`;
        case 'proxy/protocol-loaded': return `[Toolbox] Protokoll-Version ${tokens.protocolVersion} (${tokens.publisher.toUpperCase()} v${tokens.majorPatchVersion}.${tokens.minorPatchVersion}) wurde erfolgreich initialisiert.`;
        case 'proxy/error-installing-gpks': return 'FEHLER: GPK-Modul konnte nicht installiert werden! Die komplette Fehler-Nachricht lautet:';
        ...
        case 'gui/tooltip/toggleModAutoupdate': return 'Modul-Aktualisierungen aktivieren / deaktivieren';
        ...
        case 'gui/main/static/tabs/settings/content/noserverautojoin': return 'Deaktiviere automatischen Server-Beitritt';
        ...
        case 'mod/settings-load-error-corrupted-2': return `Dadurch wurden die Einstellungen für das Modul "${tokens.name}" beschädigt!`;
        case 'mod/settings-load-error-corrupted-3': return 'Das Modul lädt jetzt die Standardeinstellungen, passe sie also an deine Bedürfnisse an.';
        ...
        case 'mod/settings-load-error-invalid-format-1': return `Ungültiges Einstellungsformat für das Modul "${tokens.name}"!`;
        case 'mod/settings-load-error-invalid-format-2': return 'Dies bedeutet, dass du es beim manuellen Bearbeiten beschädigt hast.';
        case 'mod/settings-load-error-invalid-format-3': return 'Bitte korrigiere die Einstellungsdatei manuell oder lösche sie, damit die Standardeinstellungen wiederhergestellt werden können.';
        ...
        case 'mod/client-install-error-1': return `Ein Fehler ist während der Installation der Client-Komponenten aufgetreten!`;
        case 'mod/client-install-error-2': return `Bitte kontaktiere den Autor des Moduls: ${tokens.supportUrl}`;

        case 'mod/mod-preloaded': return `[Modul] Modul ${tokens.name} wurde initialisiert`;
        case 'mod/mod-preload-error-1': return `[Modul] FEHLER: Modul ${tokens.name} konnte nicht initialisiert werden!`;
        case 'mod/mod-preload-error-2': return `[Modul] FEHLER: Bitte kontaktiere den Autor des Moduls: ${tokens.supportUrl}`;
        case 'mod/mod-unloaded': return `[Modul] Modul ${tokens.name} wurde entladen`;
        case 'mod/mod-reloaded': return `[Modul] Modul ${tokens.name} wurde neu geladen`;

        case 'mod/mod-global-instance-loaded': return `[Modul] Modul ${tokens.name} wurde gestartet`;
        case 'mod/mod-global-instance-load-error-1': return `[Modul] FEHLER: Modul ${tokens.name} konnte nicht gestartet werden!`;
        case 'mod/mod-global-instance-load-error-2': return `[Modul] FEHLER: Bitte kontaktiere den Autor des Moduls: ${tokens.supportUrl}`;
        case 'mod/mod-global-instance-unloaded': return `[Modul] Modul ${tokens.name} wurde entladen`;
        case 'mod/mod-global-instance-unload-error-1': return `[Modul] FEHLER: Modul ${tokens.name} konnte nicht entladen werden!`;
        case 'mod/mod-global-instance-unload-error-2': return `[Modul] FEHLER: Bitte kontaktiere den Autor des Moduls: ${tokens.supportUrl}`;

        case 'mod/mod-client-instance-loaded': return `[Modul] Client-Modul ${tokens.name} wurde verbunden`;
        case 'mod/mod-client-instance-load-error-1': return `[Modul] FEHLER: Client-Modul ${tokens.name} konnte nicht verbunden werden!`;
        case 'mod/mod-client-instance-load-error-2': return `[Modul] FEHLER: Bitte kontaktiere den Autor des Moduls: ${tokens.supportUrl}`;
        case 'mod/mod-client-instance-unloaded': return `[Modul] Die Verbindung des Client-Moduls ${tokens.name} wurde getrennt`;
        case 'mod/mod-client-instance-unload-error-1': return `[Modul] FEHLER: Die Verbindung des Client-Moduls ${tokens.name} konnte nicht getrennt werden!`;
        case 'mod/mod-client-instance-unload-error-2': return `[Modul] FEHLER: Bitte kontaktiere den Autor des Moduls: ${tokens.supportUrl}`;

        case 'mod/mod-network-instance-loaded': return `[Modul] Netzwerk-Modul ${tokens.name} wurde verbunden`;
        case 'mod/mod-network-instance-load-error-1': return `[Modul] FEHLER: Netzwerk-Modul ${tokens.name} konnte nicht verbunden werden!`;
        case 'mod/mod-network-instance-load-error-2': return `[Modul] FEHLER: Bitte kontaktiere den Autor des Moduls: ${tokens.supportUrl}`;
        case 'mod/mod-network-instance-unloaded': return `[Modul] Verbindung vom Netzwerk-Modul ${tokens.name} wurde getrennt`;
        case 'mod/mod-network-instance-unload-error-1': return `[Modul] FEHLER: Die Verbindung des Netzwerk-Moduls ${tokens.name} konnte nicht getrennt werden!`;
        ...
        // mod-manager
        case 'mod-manager/load-module-info-error': return `[Modul] FEHLER: Modulinformationen für "${tokens.name}" können nicht geladen werden! Die vollständige Fehlermeldung lautet:`;
        case 'mod-manager/duplicate-mod-error': return `[Modul] FEHLER: Modul Duplikat "${tokens.name}" gefunden!`;
        case 'mod-manager/missing-mod-dependency-error': return `[Modul] FEHLER: Modul ${tokens.name} benötigt "${tokens.dependency}" was nicht installiert ist!`;
        case 'mod-manager/mod-conflict-error': return `[Modul] FEHLER: Modul ${tokens.name} kann nicht geladen werden, während "${tokens.conflict}" installiert ist!`;
