...
class Dispatch extends EventEmitter {
	...
	setProtocolVersion(version) {
        ...
        const protocol_data = require('tera-data/mappings.json')[this.connection.info.region];
        if (protocol_data) {
            this.region = this.connection.info.regionShort
            this.majorPatchVersion = protocol_data.major_patch
            this.minorPatchVersion = protocol_data.minor_patch

            if(this.protocolVersion !== 0 && this.protocolVersion !== protocol_data.version) {
                log.error(`[dispatch] mismatching protocol version ${this.protocolVersion}`)
                if(this.protocolVersion > protocol_data.version) {
