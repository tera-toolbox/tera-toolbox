...
class Module {
  constructor(manager, moduleInfo) {
    ...
    this.niceName = this.options.niceName || this.name;
    ...
    this.require = new Proxy(Object.create(null), {
      get: (obj, key) => {
        const mod = this.manager.load(key);
        if(!mod)
  ...
  // Default modules
  get command() { if(this._command) return this._command; return (this._command = this.require['command'].createInstance(this)); }
  get game() { if(this._game) return this._game; return (this._game = this.require['tera-game-state']); }
  ...
  setTimeout(callback, delay, ...args) {
    // TODO FIXME: workaround for critical bug in Node v11.3
    // TODO FIXME: for example delay == 880.000000000001 will cause an infinite loop...
